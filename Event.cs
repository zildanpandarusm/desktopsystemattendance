...
namespace DesktopAttendanceSystem
{
    public partial class Event : Form
    {
        ...
        private DataTable GetUsersWithRole2()
        {
            ...
            {
                query = "SELECT id, nama FROM users WHERE role = 2";
                return dbManager.ExecuteQuery(query);
            } else if(currentUserRole == 2)
            {
                int currentUserId = Login.currentUserSession.UserId;
                query = $"SELECT id, nama FROM users WHERE id = {currentUserId}"; 
                return dbManager.ExecuteQuery(query);
            } else
            {
                return null;
        ...
        public void displayData()
        {
            ...
            if (currentUserRole == 1)
            {
                query = "SELECT e.id, e.name, u.nama as nama_operator, e.date, e.location FROM events e JOIN users u ON e.iduser = u.id";
            } else if (currentUserRole == 2)
            {
                int currentUserId = Login.currentUserSession.UserId;
                query = $"SELECT e.id, e.name, u.nama as nama_operator, e.date, e.location FROM events e JOIN users u ON e.iduser = u.id WHERE e.iduser = {currentUserId}";
            }
            else
            {
